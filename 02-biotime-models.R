...
save(richness_fresh_inv, file = "data/output/richness_fresh_inv_m.RData")
...
save(richness_terr_inv, file = "data/output/richness_terr_inv_m.RData")
...
save(biomass_fresh_inv, file = "data/output/biomass_fresh_inv_m.RData")
...
save(duration_biomass2, file = "data/output/duration_biomass2.RData")
...
save(biomass_terr_inv, file = "data/output/biomass_terr_inv_m.RData")

# Calculate and save time series duration
duration_biomass <- terr.biomass %>% group_by(rarefyID) %>%
  mutate(duration = max(YEAR) - min(YEAR)) %>%
  dplyr::select(rarefyID, duration) %>% distinct()
save(duration_biomass, file = "data/output/duration_biomass.RData")
...
save(abundance_fresh_inv, file = "data/output/abundance_fresh_inv_m.RData")

# Calculate and save time series duration
duration_abundance <- fresh.abundance %>% group_by(rarefyID) %>%
  mutate(duration = max(YEAR) - min(YEAR)) %>%
  dplyr::select(rarefyID, duration) %>% distinct()
save(duration_abundance, file = "data/output/duration_abundance.RData")
...
save(abundance_terr_inv, file = "data/output/abundance_terr_inv_m.RData")

# Calculate and save time series duration
duration_abundance2 <- terr.abundance %>% group_by(rarefyID) %>%
  mutate(duration = max(YEAR) - min(YEAR)) %>%
  dplyr::select(rarefyID, duration) %>% distinct()
save(duration_abundance2, file = "data/output/duration_abundance2.RData")